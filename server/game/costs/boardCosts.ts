...
export function bowParent(): Cost {
    return new GameActionCost(GameActions.bow((context) => ({ target: context.source.attachedCharacter })));
}
...
export function removeFateFromParent(): Cost {
    return new GameActionCost(GameActions.removeFate((context) => ({ target: context.source.attachedCharacter })));
}
