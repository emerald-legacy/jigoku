        ...
        this.reaction({
            ...
            when: {
                onConflictDeclared: (event, context) =>
                    context.source.type === CardType.Attachment && (event.attackers ?? []).includes(context.source.attachedCharacter as DrawCard),
                onDefendersDeclared: (event, context) =>
                    context.source.type === CardType.Attachment && (event.defenders ?? []).includes(context.source.attachedCharacter as DrawCard)
            },
