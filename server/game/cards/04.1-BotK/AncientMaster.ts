        ...
        this.reaction({
            ...
            when: {
                onConflictDeclared: (event, context) =>
                    context.source.type === CardType.Attachment && (event.attackers ?? []).includes(context.source.parentCharacter as DrawCard),
                onDefendersDeclared: (event, context) =>
                    context.source.type === CardType.Attachment && (event.defenders ?? []).includes(context.source.parentCharacter as DrawCard)
            },
